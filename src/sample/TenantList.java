...
/**
 * Collection class to hold the list of tenants
 * @author mogaka
...
public class TenantList {
    ...
    public boolean removeTenant (int roomIn) {
        ...
        if (findT != null) {
            tList.remove(findT);
            return true;//removes given tenant
        }
        else{
     ...
     * @return Returns the tenant at the given logical position in the list or null if no tenant at the logical position
     */
    public Tenant getTenant(int positionIn) {
        if (positionIn < 1 || positionIn > getTotal()) {//checks for a valid position
    ...
    /**
     * Gets the total number of the tenants
     * @return Returns the total number of the tenants currently in the list
     */
    public int getTotal() {
        return tList.size();
    }
